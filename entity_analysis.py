...
def merge_entities(df):
    ...
    for i in df.index:
        ...
        if i % 500 == 0: print(i, i_w.lower()[0])
    ...
    # print("Number of redudant entities:", len(to_be_removed)) 
...
unique_entities = unique_entities.rename(columns={'article_id': 'no_occurrences'})  # .sort_values(by=['no_occurrences'], ascending=False)
print(unique_entities)
merged_entities = merge_entities(unique_entities)
print(unique_entities.sort_values(by=['no_occurrences'], ascending=False))
print(merged_entities.sort_values(by=['no_occurrences'], ascending=False))
count = pd.DataFrame(unique_entities.groupby('no_occurrences').size()).reset_index().rename(columns={0: 'no_words'})
