...
def create_data_frames():
    ...
    essentials_df = entities_df[
        entities_df["entity"].apply(lambda x: x in ["PER", "ORG", "LOC", "EVN"])
    ]
...
def initial_analysis(articles_df, entities_df, ambiguous_df, essentials_df):
    ...
    ambiguous_share = len(ambiguous_df.index) / (
        len(ambiguous_df.index) + len(entities_df.index)
    )
...
unique_entities = pd.DataFrame(
    dfs[3].groupby("word")["article_id"].apply(list).reset_index(name="article_ids")
)
